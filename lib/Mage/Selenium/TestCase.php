...
class Mage_Selenium_TestCase extends PHPUnit_Extensions_SeleniumTestCase
{
    ...
    /**
     * Log Out link at frontend
     * @var string
     */
    protected static $xpathLogOutFrontend = "//a[@id='logout-link']";    
...
//                'Page title is unexpected');
    ...
    public function getCurrentUimapPage()
    {
        ...
        //return $this->getUimapPage($this->getArea(), $this->_findCurrentPageFromUrl($this->getLocation())); this line will get current page from url
    }
    ...
    public function logoutCustomer()
    {
        try {
            $this->frontend('home');
            if ($this->isElementPresent(self::$xpathLogOutFrontend) && $this->isVisible(self::$xpathLogOutFrontend)) {
                $this->clickAndWait(self::$xpathLogOutFrontend, $this->_browserTimeoutPeriod);
                $this->frontend('home');
            }
        } catch (PHPUnit_Framework_Exception $e) {
            $this->fail($e->getMessage());
